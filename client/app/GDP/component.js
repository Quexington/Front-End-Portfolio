...
class GDP extends React.Component {
  ...
  componentDidMount(){
    ...
    let scaleXAxis = d3.scaleTime();
    let scaleYAxis = d3.scaleLinear();

    scaleX.domain([0, dataset.length]).range([0 + padding, width]);
    scaleY.domain([0, d3.max(dataset, (d) => d[1])]).range([0, height - padding]);
    scaleXAxis.domain([d3.min(yearsDate), d3.max(yearsDate)]).range([0 + padding, width]);
    scaleYAxis.domain([d3.max(dataset, (d) => d[1]), 0]).range([0 + padding, height]);

    const xAxis = d3.axisBottom(scaleXAxis);
    const yAxis = d3.axisLeft(scaleYAxis);

    const svg = d3.select('#gdp-subcontainer')
      .append('svg')
      .attr('width', width + padding)
      .attr('height', height + padding)

    let tooltip = d3.select("#gdp-subcontainer").append("div")
      .attr("id", "gdp-tooltip")
      .style("opacity", 0);

    let overlay = d3.select('#gdp-subcontainer').append('div')
      .attr('class', 'gdp-overlay')
      .style('opacity', 0);
    ...
    svg.selectAll('rect')
      ...
      .on('mouseover', function(d, i) {
          ...
          tooltip.transition()
            .duration(200)
            .style('opacity', .9);
          tooltip.text(dataset[i][0] + ' $' + dataset[i][1] + ' Billion')
