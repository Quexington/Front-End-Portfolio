...
class Scatterplot extends React.Component {
  ...
  componentDidMount(){
    ...
    function parseTime(time) {
      ...
      var seconds = new Date(Date.UTC(1970, 0, 1, 0, split[0], split[1]));
      return seconds;
    }
    ...
    var xScale = d3.scaleLinear().domain([d3.min(json, (d) => {
      return d.Year - 1
    }), d3.max(json, (d) => {
      return d.Year + 1
    ...
    const svg = d3.select("#scatter-subcontainer")
      .append("svg")
      .attr("height", height)
      .attr("width", width);
    var tooltip = d3.select("#scatter-subcontainer").append("div")
      .attr("id", "scatter-tooltip")
      .style("opacity", 0);

    var overlay = d3.select('#scatter-subcontainer').append('div')
      .attr('class', 'scatter-overlay')
      .style('opacity', 0);
    ...
    svg.selectAll("circle")
       ...
       .on('mouseover', (d, i) => {
          ...
          tooltip.transition()
            .duration(200)
            .style('opacity', .9);
          tooltip.text(json[i].Doping)
            .attr('data-year', json[i].Year)
        })
    ...
    svg.append("div")
       ...
       .text("Orange is without, blue is with.");
  }
  ...
  render(){
    return (
      <div id="scatter-container">
        <Back/>
        <div id="scatter-subcontainer">
          <h1 id="scatter-title">Doping in Professional Bicycle Racing</h1>
          <h2 id="scatter-description">35 Fastest times up Alpe d'Huez</h2>
        </div>
      </div>
    )
  }
